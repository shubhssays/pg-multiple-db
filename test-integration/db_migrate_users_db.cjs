...
function up() {
  exec('npm --prefix users_db run users_db-migrate-up', (error, stdout, stderr) => {
    if (stderr) {
...
function down() {
  exec('npm --prefix users_db run users_db-migrate-down', (error, stdout, stderr) => {
    if (stderr) {
      console.error(stderr);
      process.exit(1);
    }
...
function create(migration_file_name) {
  exec(`npm --prefix users_db run users_db-migrate-create ${migration_file_name}`, (error, stdout, stderr) => {
    if (stderr) {
      console.error(stderr);
      process.exit(1);
    }
...
if (action === 'up') {
  ...
  if (!migration_file_name) {
    ...
    console.error('Usage: npm run migrate:users_db:create [migration_file_name]');
    process.exit(1);
  }
